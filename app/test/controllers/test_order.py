...
def __create_items(items: list, controller: BaseController):
    ...
    for beverage in items:
        if beverage is not None:
          created_item, _ = controller.create(beverage)
          created_items.append(created_item)
...
def __create_sizes_beverages_and_ingredients(ingredients: list, beverages:list,sizes: list):
    ...
    return created_sizes if len(created_sizes) > 1 else created_sizes, created_ingredients,created_beverages


def test_create(app, ingredients, beverages, size, client_data):
    created_size, created_ingredients, created_beverages = __create_sizes_beverages_and_ingredients(ingredients, beverages, [size])
    order = __order(created_ingredients, created_beverages,created_size[0], client_data)
    created_order, error = OrderController.create(order)
    size_id = order.pop('size_id', None)
    ingredient_ids = order.pop('ingredients', [])
    beverage_id = order.pop('beverages',[])
    ...
    for param, value in order.items():
        ...
        pytest.assume(not beverages_in_detail.difference(beverage_id))
...
def test_calculate_order_price(app, ingredients, beverages, size, client_data):
    ...
    pytest.assume(created_order['total_price'] == round(created_size[0]['price'] + sum(ingredient['price'] for ingredient in created_ingredients)+sum(beverage['price'] for beverage in created_beverages), 2))
...
def test_get_all(app, ingredients, beverages, sizes, client_data):
    created_sizes, created_ingredients, created_beverages = __create_sizes_beverages_and_ingredients(ingredients, beverages, sizes)
    created_orders = []
    for _ in range(5):
